...
class EnhancedSearchService {
  ...
  async search(searchQuery: SearchQuery, useDebouncing = true): Promise<SearchOperationResult<SearchResults>> {
    ...
    try {
      console.log('🔍 SEARCH: Starting search with query:', {
      ...
      if (!validation.valid) {
        console.error('❌ SEARCH: Query validation failed:', validation.errors);
        throw new Error(`Invalid query: ${validation.errors.join(', ')}`);
      }

      // Use debounced search for real-time queries
      if (useDebouncing && searchQuery.term.trim()) {
        console.log('⏱️ SEARCH: Using debounced search');
        const results = (await this.debouncedSearch.execute(searchQuery.term, searchQuery)) as SearchResults;
        const responseTime = Date.now() - startTime;

        console.log('✅ SEARCH: Debounced search completed', {
      ...
      console.log('🚀 SEARCH: Using immediate search (no debouncing)');
  ...
  private async _executeSearchInternal(searchQuery: SearchQuery): Promise<SearchResults> {
    ...
    console.log('🎯 SEARCH_INTERNAL: Executing search internally');

    // Handle 'all' search type by searching multiple collections
    if (searchType === 'all') {
      console.log('🔄 SEARCH_INTERNAL: Searching all collections');
      return this._searchAllCollections(searchQuery);
    }

    // Handle boolean operators if present
    if (fuzzyMatcher.hasBooleanOperators(term)) {
      console.log('📝 SEARCH_INTERNAL: Detected boolean operators, using boolean search');
      return this._executeBooleanSearch(searchQuery);
    }

    // Build and execute query
    console.log('🏗️ SEARCH_INTERNAL: Building Firestore query');
    const firestoreQuery = queryBuilder.buildQuery(searchQuery);
    console.log('📡 SEARCH_INTERNAL: Executing Firestore query');
    const querySnapshot = await getDocs(firestoreQuery);

    console.log('📦 SEARCH_INTERNAL: Query returned', querySnapshot.size, 'documents');

    // Process results
    const items = this._processQueryResults(querySnapshot, searchQuery);
    console.log('✅ SEARCH_INTERNAL: Processed', items.length, 'items');
    ...
    console.log('🔎 SEARCH_INTERNAL: After fuzzy matching:', filteredItems.length, 'items');
    ...
    console.log('📊 SEARCH_INTERNAL: Final results:', {
