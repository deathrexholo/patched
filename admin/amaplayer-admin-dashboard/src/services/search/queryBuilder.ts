...
class QueryBuilder {
  ...
  buildQuery(searchQuery: SearchQuery, lastDoc?: DocumentData): Query<DocumentData> {
    ...
    // Build query constraints array (all at once, not sequentially)
    const queryConstraints: any[] = [];

    // Add all where constraints
    ...
    for (const orderConstraint of orderConstraints) {
      queryConstraints.push(orderBy(orderConstraint.field, orderConstraint.direction));
    }
    ...
    // Build final query with all constraints at once
